...
class NaiveBayes:
  ...
  class kthClassCount:
    def __init__(self,klass,words):
        self.name=klass
        self.wordsCount=collections.defaultdict(lambda:0)
        self.totalExample=0
        self.total=0.0
        for word in words:
  ...
  def classify(self, words):
    agrmaxprob=float('-inf')
    agrmaxlabel='pos'
    totalNumber=0.0
    for classCount in self.classCounts:
        totalNumber+=classCount.totalExample
    for classCount in self.classCounts:
        prob=0.0
        label=classCount.name
        prob=math.log(classCount.totalExample)-math.log(totalNumber)
        for word in words:
            prob+=math.log(classCount.wordsCount[word]+1)-math.log(len(classCount.wordsCount)+classCount.total)
        if prob>agrmaxprob:
            agrmaxprob=prob
            agrmaxlabel=label
    return agrmaxlabel
  ...
  def addExample(self, klass, words):
      flag=True
      for classCount in self.classCounts:
          if klass==classCount.name:
             flag=False
             classCount.add(words)
      if flag==True:
         self.classCounts.append(self.kthClassCount(klass,words))
